...
namespace TDnRFLib //TDnRFcontroller_LIB
{
    ...
    public class UpDnEngine
    {
        ...
        public Int32 Dn_Send_Dcmd(byte[] p_buf, int len)// array<System::Byte,1> p_buf, int len)
        {
            ...
            try
            {
                masterEmulator.SendData(pipeSetup.DcmdPipe, p_buf); //TODO if pipe not open get an exception
            }
            catch
            {
                r = -1;
        ...
        public Int32 Dn_Send_Ddat(byte[] p_buf, int len)//  array<System::Byte,1> p_buf, int len)
        {
            ...
            masterEmulator.SendData(pipeSetup.DdatPipe, p_buf); //TODO if pipe not open get an exception
            return (r);
        }
        //---------------------------------------------------------------------
        ...
        */
        ...
        Int32 blk_dn_start(byte[] pkt)// array<Byte,1> pkt )
        {
            ...
            /*TODO*/
            // if m_blkDn_blkCnt > BLK_DN_COUNT we get an index out of range!!!
            /*TODO*/
        ...
        public Int32 Un_Send_Ucfm(byte[] p_buf, int len) // array<System::Byte,1> p_buf, int len)
        {
            ...
            masterEmulator.SendData(pipeSetup.UcfmPipe, p_buf); //TODO if pipe not open get an exception
            return (r);
        }
    ...
    public class CtrlEngine
    {
        ...
        Int32 UpDn_Write_Wctrl_test(Byte[] p_buf, int len)//array<System::Byte,1> p_buf, int len)
        {
            ...
            bret = masterEmulator.SendData(pipeSetup.WctrlPipe, p_buf); //TODO if pipe not open get an exception
            if (bret == true)
            {
    ...
    public class Initialized_OK_EventArgs : EventArgs
    {
        public bool Status { get; set; }

        public Initialized_OK_EventArgs(bool status)
        {
            Status = status;
        }
    }
    ...
    /// logic.
    /// </summary>
    public partial class TDnRFcontroller //#1 Initialization
    {
        /* Event declarations */
        //*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        public event EventHandler<Initialized_OK_EventArgs> appEv_InitializedResult;
        ...
        //*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        //*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        public bool hasBLEMasterDevice { get; private set; } // Dongle is plugged in or not
        public bool IsBusy_Initializing { get; private set; }
        public bool IsInitialized { get; private set; }
        public bool Initialized_OK { get; private set; }
        public bool IsBleOn
        {
            get
            {
                if (!hasBLEMasterDevice) return (false); // No Dongle
                if (!IsInitialized) return (false);      // Not Initialized yet
                if (!Initialized_OK) return (false);     // Initialized but somthing wrong
                return (true);
            }
            //private set;
        }
        ...
        //*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        //*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        public bool IsBusy_Scanning { get; private set; }
        public bool IsBusy_StopScanning { get; private set; }
        void _init_Scan()
        {
            IsBusy_Scanning = false;
            IsBusy_StopScanning = false;
        }

        //*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        //*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*
        public bool IsBusy_Connecting { get; private set; }
        public bool IsBusy_Disconnecting { get; private set; }
        public bool STATE_connectionInProgress;
        public bool FLAG_autoConnect;
        public String m_autoConnect_sDeviceId;
        void _init_Connect()
        {
            IsBusy_Connecting = false;
            IsBusy_Disconnecting = false;
            STATE_connectionInProgress = false;
        ...
        static private TDnRFcontroller instance = null;
        static public TDnRFcontroller getInstance()
        {
            if (instance == null)
                instance = new TDnRFcontroller();
            return (instance);
        }
        ...
        string FindUsbDevice()
        {
            ...
            var devices = masterEmulator.EnumerateUsb(UsbDeviceType.MasterEmulatorJlinkDongle);
    ...
    public partial class TDnRFcontroller //#2 Scanning
    {
    ...
    public partial class TDnRFcontroller //#3 Connecting
    {
        //=====================================================================
        //=====================================================================
        //=====================================================================
        ...
        public bool Connect_to_device_fromText(String sDeviceAddr)
        {
            bool bStatus;
            ...
            DeviceAddress = new BtDeviceAddress(sDeviceAddr);
            ...
            STATE_connectionInProgress = true;
            bStatus = masterEmulator.Connect(DeviceAddress, connectionParams);
            if (bStatus == false)
            {
                STATE_connectionInProgress = false;
                IsBusy = false;
        ...
        void meEv_OnConnected(object sender, EventArgs arguments)
        {
            ...
            */
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                {
                    LogErrorMessage(string.Format("Exception in On_meEv_Connected: {0}", ex.Message),  ex.StackTrace);
                    IsBusy_Connecting = false;
                    IsBusy = false;
                }
            });
    ...
    public partial class TDnRFcontroller //#4 Characteristic Operations
    {
        //=====================================================================
        //=====================================================================
        //=====================================================================
...
#if false // example  UARTStartSendData(byte[] data)
    ...
    public partial class TDnRFcontroller //#5 House keeping
    {
        //=====================================================================
        //=====================================================================
        //=====================================================================
    ...
    public class SunaBLE
    {
        TDnRFcontroller bleCtrl;
        TDnRFcontrollerForm_1 form = null;
        Form m_parentForm;


        public SunaBLE(Form parentForm)
        {
            m_parentForm = parentForm;
            
            // I keep on fucking up with multiple instances, so use a singleton
            // bleCtrl = new TDnRFcontroller();
        ...
        public void ShowForm()
        {
            if (form == null)
            {
                form = new TDnRFcontrollerForm_1(m_parentForm);
                //form.local_bleInit(); //XXXXXXXXXX
            }
            else
                if(form.IsDisposed)
                    form = new TDnRFcontrollerForm_1(m_parentForm);
        ...
        //---------------------------------------------------------------------
        Byte[] theRxPacket;
        Int32 theRxPacketLength;
        void bleCtrl_userEv_udPacket(object sender, udPacketEventArgs e)
        {
            theRxPacket = e.Packet;
            theRxPacketLength = e.Length;
        }

        bool _IsOpen = false;

        //---------------------------------------------------------------------
        public Int32 rtrBle_Open()
        {
            if (!_IsOpen)
            {
                _IsOpen = true;
                //bleCtrl.Connect_to_device(null);
                //bleCtrl.udEngine.set_rxCallback(local_ON_udPacket);
                bleCtrl.userEv_udPacket += bleCtrl_userEv_udPacket;
            }
            return (0);
        }
        //---------------------------------------------------------------------
        public Int32 rtrBle_Close()
        {
            if (_IsOpen)
            {
                _IsOpen = false;
        ...
        //---------------------------------------------------------------------
        public bool rtrBle_IsOpen()
        {
            return (_IsOpen);
            /*
            if (bleCtrl.STATE_connectionInProgress == true)
                return (true);
            else
                return (false);
            */
        ...
        //---------------------------------------------------------------------
        public bool rtrBle_Write_Packet(Byte[] packet)
        {
            Int32 r;
            if (_IsOpen)
            {
                //r = bleCtrl.PC_Send_udPacket(packet);
                r = bleCtrl.udEngine.Dn_Send_CMD_11_Pkt(packet);
            }
        ...
        //---------------------------------------------------------------------
        public bool rtrBle_Read_Packet(ref Byte[] packet)
        {
            if (theRxPacket == null)
            {
                return (false);
            ...
            {
                packet = new Byte[theRxPacket.Length];
                Array.Copy(theRxPacket, packet, packet.Length);
